...
# ================================================================
tolerance_colors = ["BROWN", "RED", "ORANGE", "YELLOW" , "GREEN", "BLUE", "VIOLET", "GREY", "GOLD", "SILVER"]
...
def color_to_value(color):    
    for i in range(0,len(valid_colors)):
        if color == valid_colors[i] and i<10:
            return i
            break
...
def main():
   ...
   for i in bands: 
       if i not in valid_colors:
           print("Invalid Color")
           return
   if bands[0] in ["GOLD","SILVER"] or bands[3] == "WHITE":
        print("WRONG ORDER, TURN THE RESISTOR AROUND") 
        return compute_resistor_value(color_to_value(bands[3]), color_to_value(bands[2]),  color_to_multiplier_and_tolerance(bands[1],bands[0])[0], color_to_multiplier_and_tolerance(bands[1],bands[0])[1])   
   if bands[0] in ["BLACK","GOLD","SILVER"] or bands[1] in ["GOLD","SILVER"]:
       print("INVALID DIGIT")
       return
